...
const preprocessThinkTag = (content: string) => {
  ...
  return flow([
    (str: string) =>
      str.replace(thinkOpenTagRegex, "<details data-think=true>\n"),
    (str: string) =>
      str.replace(thinkCloseTagRegex, "\n[ENDTHINKFLAG]</details>"),
  ])(content);
};
...
const CodeBlock: any = memo(
  ({ inline, className, children = "", ...props }: any) => {
    ...
    const echartsOnEvents = useMemo(
      ...
      [echartsRef]
    ); // echartsRef is stable, so this effectively runs once.
    ...
    const renderCodeContent = useMemo(() => {
      ...
      switch (language) {
        case "mermaid":
          if (isSVG) return <Flowchart PrimitiveCode={content} />;
          break;
        case "echarts": {
        ...
        default:
          return (
            <SyntaxHighlighter
              {...props}
              style={
                theme === Theme.light ? atelierHeathLight : atelierHeathDark
              }
              customStyle={{
                paddingLeft: 12,
                borderBottomLeftRadius: "10px",
...
const VideoBlock: any = memo(({ node }: any) => {
  ...
  if (srcs.length === 0) {
    ...
    if (src) return <VideoGallery key={src} srcs={[src]} />;
    return null;
  }
...
const Link = ({ node, children, ...props }: any) => {
  if (
    ...
    const hidden_text = decodeURIComponent(
      node.properties.href.toString().split("abbr:")[1]
    );
...
export function Markdown(props: {
  ...
  const latexContent = flow([preprocessThinkTag, preprocessLaTeX])(
    props.content
  );

  return (
    <div className={cn("markdown-body", "!text-white", props.className)}>
      <ReactMarkdown
        remarkPlugins={[
          RemarkGfm,
